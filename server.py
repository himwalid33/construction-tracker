...
# Use environment variable for DB path (for Render Disk), fallback to local
DB_PATH = os.environ.get('DB_PATH', os.path.join(os.path.dirname(__file__), 'construction.db'))
...
# Initialize database on module load (works with Gunicorn on Render)
init_db()
